...
export const resolver: QueryResolvers['recordings'] = async (_parent, _args, context, _info) => {
  const file = await getRecordingFile();
  const recordingsFromFile = file.get();
  const recordings = recordingsFromFile && Object.values(recordingsFromFile).length
